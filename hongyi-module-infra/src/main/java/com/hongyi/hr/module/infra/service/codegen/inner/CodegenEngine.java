package com.hongyi.hr.module.infra.service.codegen.inner;
...
import com.hongyi.hr.framework.apilog.core.enums.OperateTypeEnum;
...
import com.hongyi.hr.framework.common.util.collection.CollectionUtils;
import com.hongyi.hr.framework.common.util.date.DateUtils;
import com.hongyi.hr.framework.common.util.date.LocalDateTimeUtils;
import com.hongyi.hr.framework.common.util.object.BeanUtils;
import com.hongyi.hr.framework.common.util.object.ObjectUtils;
import com.hongyi.hr.framework.common.util.string.StrUtils;
...
import com.hongyi.hr.module.infra.dal.dataobject.codegen.CodegenColumnDO;
import com.hongyi.hr.module.infra.dal.dataobject.codegen.CodegenTableDO;
import com.hongyi.hr.module.infra.enums.codegen.CodegenFrontTypeEnum;
import com.hongyi.hr.module.infra.enums.codegen.CodegenSceneEnum;
import com.hongyi.hr.module.infra.enums.codegen.CodegenTemplateTypeEnum;
import com.hongyi.hr.module.infra.enums.codegen.CodegenVOTypeEnum;
import com.hongyi.hr.module.infra.framework.codegen.config.CodegenProperties;
...
public class CodegenEngine {
    ...
    /**
     * 是否为 hongyi-cloud 项目，用于解决 Boot 和 Cloud 的 api 模块兼容性问题
     *
     * true  - 需要有 hongyi-module-xxx-api 模块
    ...
    public CodegenEngine() {
        ...
        this.cloudEnable = ClassUtils.isPresent("com.hongyi.hr.module.infra.framework.rpc.config.RpcConfiguration",
    ...
    private Map<String, String> getTemplates(Integer frontType) {
        ...
        if (Boolean.FALSE.equals(cloudEnable)) {
            SERVER_TEMPLATES.forEach((templatePath, filePath) -> {
                filePath = StrUtil.replace(filePath, "/hongyi-module-${table.moduleName}-api", "");
                filePath = StrUtil.replace(filePath, "/hongyi-module-${table.moduleName}-server", "");
    ...
    private static String javaModuleFilePath(String path, String module, String src) {
        return "hongyi-module-${table.moduleName}/" + // 顶级模块
                "hongyi-module-${table.moduleName}-" + module + "/" + // 子模块
                "src/" + src + "/java/${basePackage}/module/${table.moduleName}/" + path + ".java";
    }

    private static String mapperXmlFilePath() {
        return "hongyi-module-${table.moduleName}/" + // 顶级模块
                "hongyi-module-${table.moduleName}-server/" + // 子模块
    ...
    private static String vueFilePath(String path) {
        return "hongyi-ui-${sceneEnum.basePackage}-vue2/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3FilePath(String path) {
        return "hongyi-ui-${sceneEnum.basePackage}-vue3/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3UniappFilePath(String path) {
        return "hongyi-ui-${sceneEnum.basePackage}-uniapp/" + // 顶级目录
                "src/" + path;
    }

    private static String vue3VbenFilePath(String path) {
        return "hongyi-ui-${sceneEnum.basePackage}-vben/" + // 顶级目录
                "src/" + path;
    }
